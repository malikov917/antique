...
ARTIFACT_DIR="${ARTIFACT_ROOT}/ios-buyer-offer-from-feed-${RUN_STAMP}"
...
require_command() {
  ...
  if ! command -v "$command_name" >/dev/null 2>&1; then
    echo "[buyer-offer-from-feed] missing command: $command_name"
    return 1
  fi
}
...
if ! require_command maestro; then
  echo "[buyer-offer-from-feed] install Maestro: https://maestro.mobile.dev/getting-started/installing-maestro"
  exit 1
fi

if ! require_command xcrun; then
  echo "[buyer-offer-from-feed] xcrun is required for iOS simulator checks"
  exit 1
fi

if ! xcrun --find simctl >/dev/null 2>&1; then
  echo "[buyer-offer-from-feed] xcrun simctl is unavailable"
  exit 1
fi

if ! xcrun simctl list devices booted | grep -q "(Booted)"; then
  echo "[buyer-offer-from-feed] no booted iOS simulator found"
  echo "[buyer-offer-from-feed] boot simulator first, then run this command again"
  exit 1
fi
...
if [[ -z "$IOS_DEVICE_ID" ]]; then
  echo "[buyer-offer-from-feed] could not resolve a booted iPhone simulator UDID"
  exit 1
fi

echo "[buyer-offer-from-feed] lint/typecheck/test"
pnpm check

echo "[buyer-offer-from-feed] build api"
pnpm build

echo "[buyer-offer-from-feed] start api"
...
echo "[buyer-offer-from-feed] setup API workflow fixtures"
if ! pnpm --filter @antique/api exec node scripts/workflow-e2e-setup.mjs \
  ...
  echo "[buyer-offer-from-feed] failed during workflow setup"
  echo "[buyer-offer-from-feed] artifacts: $ARTIFACT_DIR"
  exit 1
fi

BUYER_ACCESS_TOKEN="$(node -e 'const fs=require("fs"); const payload=JSON.parse(fs.readFileSync(process.argv[1],"utf8")); process.stdout.write(payload.buyerAccessToken || "");' "$SETUP_JSON_FILE")"
if [[ -z "$BUYER_ACCESS_TOKEN" ]]; then
  OBSERVED_UI_ERROR="workflow setup did not produce buyerAccessToken"
  write_summary "failed"
  echo "[buyer-offer-from-feed] setup output missing buyerAccessToken"
  echo "[buyer-offer-from-feed] artifacts: $ARTIFACT_DIR"
  exit 1
fi

echo "[buyer-offer-from-feed] start metro (buyer token)"
EXPO_PUBLIC_API_BASE_URL="http://127.0.0.1:${API_PORT}" \
EXPO_PUBLIC_ACCESS_TOKEN="$BUYER_ACCESS_TOKEN" \
...
echo "[buyer-offer-from-feed] run maestro flow"
if maestro test "e2e/maestro/ios-buyer-offer-from-feed.yaml" --device "$IOS_DEVICE_ID" -e EXPO_DEV_URL="$EXPO_DEV_URL" 2>&1 | tee -a "$MAESTRO_LOG_FILE"; then
  echo "buyer-offer-from-feed=passed" >> "$FLOW_RESULT_FILE"
else
  echo "buyer-offer-from-feed=failed" >> "$FLOW_RESULT_FILE"
  capture_failure_artifacts
  write_summary "failed"
  echo "[buyer-offer-from-feed] artifacts: $ARTIFACT_DIR"
  exit 1
...
echo "[buyer-offer-from-feed] all green"
echo "[buyer-offer-from-feed] artifacts: $ARTIFACT_DIR"
echo "[buyer-offer-from-feed] summary: $SUMMARY_JSON_FILE"
